...
BEGIN
  -- 1. Buscar al usuario de Shantal
  SELECT id INTO v_user_id FROM auth.users WHERE email = 'sgamboa@uin.ac.cr' LIMIT 1;
  
  IF v_user_id IS NULL THEN
    RAISE EXCEPTION 'El usuario sgamboa@uin.ac.cr no existe. Debes crearlo primero o registrarte.';
  END IF;

  -- ==========================================
  -- INSTALACIÓN DE SHANTAL 2026
  -- ==========================================
  INSERT INTO public.conjuntos (user_id, name) VALUES (v_user_id, 'Shantal 2026') RETURNING id INTO new_conjunto_id;
...
Código de acceso: sF39FW98');
  ...
  -- ==========================================
  -- INSTALACIÓN DE GUÍA OFICIAL CRM 2026
  -- (Usamos la función ya existente para la guía oficial, pero invocándola explícitamente para el usuario)
  -- En vez de llamar a la función, inserto los datos de forma directa para asegurar la estructura
  -- ==========================================
  
  -- Para la guía oficial usaré un script separado o aquí mismo. Vamos a extraerlo de setup_database.sql

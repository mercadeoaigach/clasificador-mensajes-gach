...
BEGIN
  -- Buscar el ID del usuario Shantal
  SELECT id INTO shantal_user_id FROM auth.users WHERE email = 'sgamboa@uin.ac.cr' LIMIT 1;
  
  -- Borrar el conjunto existente para limpiarlo totalmente
  DELETE FROM public.conjuntos WHERE name = 'Shantal 2026' AND user_id = shantal_user_id;

  -- Crear conjunto
  INSERT INTO public.conjuntos (user_id, name) VALUES (shantal_user_id, 'Shantal 2026') RETURNING id INTO new_conjunto_id;

-- SCRIPT 3: INSTALACIÓN DEL CONJUNTO "GUÍA OFICIAL" (V3 HTML)
DO $$
DECLARE
  v_user_id UUID;
...
BEGIN
  ...
  IF v_user_id IS NULL THEN RAISE EXCEPTION 'El usuario sgamboa@uin.ac.cr no existe en auth.users.'; END IF;
  INSERT INTO public.conjuntos (user_id, name) VALUES (v_user_id, 'Guía Oficial') RETURNING id INTO new_conjunto_id;

  -- CATEGORÍA: Bienvenida y datos generales
  INSERT INTO public.categorias (conjunto_id, name, color, icon) VALUES (new_conjunto_id, 'Bienvenida y datos generales', 'bg-indigo', 'map-pin');
  SELECT id INTO cat_id FROM public.categorias WHERE conjunto_id = new_conjunto_id AND name = 'Bienvenida y datos generales';
...
¿Podrías contarme qué carrera te interesa?', false, NULL);
  SELECT id INTO cat_id FROM public.categorias WHERE conjunto_id = new_conjunto_id AND name = 'Bienvenida y datos generales';
  INSERT INTO public.mensajes (categoria_id, sub_categoria, title, excerpt, is_pinned, internal_note) VALUES
  (cat_id, NULL, 'Solicitud de datos a nuevos', '✨ En la UIN queremos brindarte una atención más ágil y personalizada.
...
Así podés estudiar con total comodidad, mientras la Universidad Isaac Newton (UIN) te ayuda a construir tu futuro profesional 🍎.', false, NULL);
  SELECT id INTO cat_id FROM public.categorias WHERE conjunto_id = new_conjunto_id AND name = 'Bienvenida y datos generales';
  INSERT INTO public.mensajes (categoria_id, sub_categoria, title, excerpt, is_pinned, internal_note) VALUES
  (cat_id, NULL, 'Complemento: carrera con laboratorios', 'Además, es importante que tomés en cuenta que esta carrera incluye algunos cursos con laboratorio, los cuales son presenciales 🔬🧪.
...
Este formato te permite llevar la carrera con mucha flexibilidad, aprovechando las clases en línea y asistiendo solo cuando realmente es necesario.', false, 'Agregar este texto a continuación de D04 únicamente si la carrera consultada incluye cursos con laboratorio.');

  -- CATEGORÍA: Periodos y modalidad
  INSERT INTO public.categorias (conjunto_id, name, color, icon) VALUES (new_conjunto_id, 'Periodos y modalidad', 'bg-pink', 'user-plus');
  SELECT id INTO cat_id FROM public.categorias WHERE conjunto_id = new_conjunto_id AND name = 'Periodos y modalidad';
  INSERT INTO public.mensajes (categoria_id, sub_categoria, title, excerpt, is_pinned, internal_note) VALUES
  (cat_id, NULL, 'Matrícula ordinaria — nuevo ingreso o reingreso', '📅 Periodo de matrícula ordinaria para nuevos ingresos o reingresos:
...
🎓 Inicio de lecciones: 31 de agosto del 2026', false, 'Actualizar las fechas en verde cada cuatrimestre.');
  SELECT id INTO cat_id FROM public.categorias WHERE conjunto_id = new_conjunto_id AND name = 'Periodos y modalidad';
  INSERT INTO public.mensajes (categoria_id, sub_categoria, title, excerpt, is_pinned, internal_note) VALUES
  (cat_id, NULL, 'Matrícula ordinaria — estudiantes activos', '📚 Periodo de matrícula ordinaria para estudiantes activos:
🗓️ Del 24 al 29 de agosto del 2026
🎓 Inicio de lecciones: 31 de agosto del 2026', false, NULL);
  SELECT id INTO cat_id FROM public.categorias WHERE conjunto_id = new_conjunto_id AND name = 'Periodos y modalidad';
  INSERT INTO public.mensajes (categoria_id, sub_categoria, title, excerpt, is_pinned, internal_note) VALUES
  (cat_id, NULL, 'Matrícula extraordinaria (con recargo)', '⏳ Periodo de matrícula extraordinaria (con recargo):
🗓️ Del 31 de agosto al 6 de setiembre del 2026', false, NULL);
  SELECT id INTO cat_id FROM public.categorias WHERE conjunto_id = new_conjunto_id AND name = 'Periodos y modalidad';
  INSERT INTO public.mensajes (categoria_id, sub_categoria, title, excerpt, is_pinned, internal_note) VALUES
  (cat_id, NULL, 'Matrícula súper extraordinaria (con recargo)', '⏳ Periodo de matrícula súper extraordinaria (con recargo):
🗓️ Del 7 al 11 de setiembre del 2026', false, 'Aplica solo si el estudiante no ha cursado 2 sesiones de clase, es decir, si únicamente perdió 1 lección.');
  SELECT id INTO cat_id FROM public.categorias WHERE conjunto_id = new_conjunto_id AND name = 'Periodos y modalidad';
  INSERT INTO public.mensajes (categoria_id, sub_categoria, title, excerpt, is_pinned, internal_note) VALUES
  (cat_id, NULL, 'Modalidad de estudio', 'Nuestra modalidad está diseñada para impulsar tus metas con la flexibilidad que necesitas y la calidad que te mereces. 💫
...
✔️ Presencialidad obligatoria únicamente en los cursos de laboratorio, garantizando el aprendizaje práctico que te prepara para el mundo real, según el cronograma que en cada caso se facilite.', false, NULL);
  ...
  -- CATEGORÍA: Financiamiento y costos
  INSERT INTO public.categorias (conjunto_id, name, color, icon) VALUES (new_conjunto_id, 'Financiamiento y costos', 'bg-emerald', 'book-open');
  SELECT id INTO cat_id FROM public.categorias WHERE conjunto_id = new_conjunto_id AND name = 'Financiamiento y costos';
  INSERT INTO public.mensajes (categoria_id, sub_categoria, title, excerpt, is_pinned, internal_note) VALUES
  (cat_id, NULL, 'Financiamiento Todo Terreno', '🔹 Financiamiento Todo Terreno
...
Únicamente en el primer pago se agrega un monto de ₡4 500 correspondiente al uso de herramientas tecnológicas 🛠️.', false, NULL);
  SELECT id INTO cat_id FROM public.categorias WHERE conjunto_id = new_conjunto_id AND name = 'Financiamiento y costos';
  INSERT INTO public.mensajes (categoria_id, sub_categoria, title, excerpt, is_pinned, internal_note) VALUES
  (cat_id, NULL, 'Financiamiento 1, 2, 3', '🔹 Financiamiento 1, 2, 3
...
El 75% restante se financia en 3 tractos iguales 📊📆.', false, NULL);
  SELECT id INTO cat_id FROM public.categorias WHERE conjunto_id = new_conjunto_id AND name = 'Financiamiento y costos';
  INSERT INTO public.mensajes (categoria_id, sub_categoria, title, excerpt, is_pinned, internal_note) VALUES
  (cat_id, NULL, 'Beneficios del financiamiento y fechas de pago', '✔ Beneficios importantes
• No requiere fiador
...
Si desea, puedo brindarle una simulación de pagos según la carrera que le interesa. Será un gusto acompañarle en este proceso para que la UIN te acompañe a Construir Tu Futuro 🚀💙.', false, 'Actualizar el cuatrimestre y las fechas de pago (en verde) cada término.');
  SELECT id INTO cat_id FROM public.categorias WHERE conjunto_id = new_conjunto_id AND name = 'Financiamiento y costos';
  INSERT INTO public.mensajes (categoria_id, sub_categoria, title, excerpt, is_pinned, internal_note) VALUES
  (cat_id, NULL, 'Si manifiesta interés en matricular', '🤩 Si gustás, podés indicarme cuántas materias te interesa llevar y si preferís optar por nuestro financiamiento o pagar de contado.
...
Con esa información puedo darte el monto exacto que tendrías que cancelar al matricular y el valor de cada cuota.', false, 'Si el prospecto responde afirmativamente (ej. "sí", "dale") sin especificar cantidad de materias ni forma de pago, continuá con esta plantilla y luego pasá a D15 solicitando esos dos datos antes de dar montos.');
  SELECT id INTO cat_id FROM public.categorias WHERE conjunto_id = new_conjunto_id AND name = 'Financiamiento y costos';
  INSERT INTO public.mensajes (categoria_id, sub_categoria, title, excerpt, is_pinned, internal_note) VALUES
  (cat_id, NULL, 'Detalle de montos (servicio al cliente contesta)', 'Para [XX] materias, deberías cancelar al momento de matricular un monto de ₡[monto].
...
Estos beneficios te permiten avanzar con tranquilidad y le permiten a la Universidad Isaac Newton acompañarte a construir tu futuro 🚀💙.', false, NULL);
  ...
  INSERT INTO public.categorias (conjunto_id, name, color, icon) VALUES (new_conjunto_id, 'Requisitos de matrícula', 'bg-orange', 'credit-card');
  SELECT id INTO cat_id FROM public.categorias WHERE conjunto_id = new_conjunto_id AND name = 'Requisitos de matrícula';
  INSERT INTO public.mensajes (categoria_id, sub_categoria, title, excerpt, is_pinned, internal_note) VALUES
  (cat_id, NULL, 'Requisitos Bachillerato', '📚 Bachillerato
...
• Fotografía tamaño pasaporte o selfie frontal y formal.', false, NULL);
  SELECT id INTO cat_id FROM public.categorias WHERE conjunto_id = new_conjunto_id AND name = 'Requisitos de matrícula';
  INSERT INTO public.mensajes (categoria_id, sub_categoria, title, excerpt, is_pinned, internal_note) VALUES
  (cat_id, NULL, 'Requisitos Licenciatura', '🎓 Licenciatura
...
• Fotografía tamaño pasaporte o selfie frontal y formal.', false, NULL);
  SELECT id INTO cat_id FROM public.categorias WHERE conjunto_id = new_conjunto_id AND name = 'Requisitos de matrícula';
  INSERT INTO public.mensajes (categoria_id, sub_categoria, title, excerpt, is_pinned, internal_note) VALUES
  (cat_id, NULL, 'Requisitos Maestría', '🎓 Maestría
...
• Documento que compruebe haber realizado 150 horas de TCU.', false, NULL);
  ...
  -- CATEGORÍA: Costos por carrera
  INSERT INTO public.categorias (conjunto_id, name, color, icon) VALUES (new_conjunto_id, 'Costos por carrera', 'bg-blue', 'file-text');
  SELECT id INTO cat_id FROM public.categorias WHERE conjunto_id = new_conjunto_id AND name = 'Costos por carrera';
  INSERT INTO public.mensajes (categoria_id, sub_categoria, title, excerpt, is_pinned, internal_note) VALUES
  (cat_id, NULL, 'Cursos con laboratorio — costo', 'Los cursos con laboratorio son:
...
Estos tienen un costo de ₡174 616,90', false, NULL);
  SELECT id INTO cat_id FROM public.categorias WHERE conjunto_id = new_conjunto_id AND name = 'Costos por carrera';
  INSERT INTO public.mensajes (categoria_id, sub_categoria, title, excerpt, is_pinned, internal_note) VALUES
  (cat_id, NULL, 'Giras académicas — Ingeniería Industrial', 'Los cursos de Ingeniería Industrial con giras académicas son:
...
Estas tienen un costo de ₡87 308,45 más ₡30 000,00 de los costos de la gira (incluye transporte, póliza estudiantil y merienda)', false, NULL);
  SELECT id INTO cat_id FROM public.categorias WHERE conjunto_id = new_conjunto_id AND name = 'Costos por carrera';
  INSERT INTO public.mensajes (categoria_id, sub_categoria, title, excerpt, is_pinned, internal_note) VALUES
  (cat_id, NULL, 'Giras académicas — Ingeniería Civil', 'Los cursos de Ingeniería Civil con giras académicas son:
• Contaminación Ambiental (bachillerato)
...
Estas tienen un costo de ₡87 308,45 más ₡30 000,00 de los costos de la gira (incluye transporte, póliza estudiantil y merienda)', false, NULL);
  SELECT id INTO cat_id FROM public.categorias WHERE conjunto_id = new_conjunto_id AND name = 'Costos por carrera';
  INSERT INTO public.mensajes (categoria_id, sub_categoria, title, excerpt, is_pinned, internal_note) VALUES
  (cat_id, NULL, 'Materias con dos clases semanales — costo', 'Materias con dos clases semanales tienen un costo de ₡174 616,90:
...
• Ecuaciones Diferenciales', false, NULL);
  SELECT id INTO cat_id FROM public.categorias WHERE conjunto_id = new_conjunto_id AND name = 'Costos por carrera';
  INSERT INTO public.mensajes (categoria_id, sub_categoria, title, excerpt, is_pinned, internal_note) VALUES
  (cat_id, NULL, 'Costo Bachillerato — nuevo ingreso / reingreso', 'Para este cuatrimestre, la Universidad Isaac Newton ha habilitado beneficios especiales para los estudiantes de primer ingreso en Bachillerato en Ingeniería, con el objetivo de facilitar su inicio académico:
...
Si deseás asegurar tu cupo, puedo ayudarte a completar tu proceso de matrícula de inmediato.', false, NULL);
  SELECT id INTO cat_id FROM public.categorias WHERE conjunto_id = new_conjunto_id AND name = 'Costos por carrera';
  INSERT INTO public.mensajes (categoria_id, sub_categoria, title, excerpt, is_pinned, internal_note) VALUES
  (cat_id, NULL, 'Costo Bachillerato — activo / regular', '📘 Si sos estudiante activo, este cuatrimestre es una oportunidad ideal para seguir avanzando y no detener tu proceso académico.
...
Si querés, puedo calcularte el total a pagar según las materias que deseás llevar.', false, NULL);
  SELECT id INTO cat_id FROM public.categorias WHERE conjunto_id = new_conjunto_id AND name = 'Costos por carrera';
  INSERT INTO public.mensajes (categoria_id, sub_categoria, title, excerpt, is_pinned, internal_note) VALUES
  (cat_id, NULL, 'Costo Licenciatura — nuevo ingreso / reingreso', 'Para este cuatrimestre, la Universidad Isaac Newton ha habilitado beneficios especiales para los estudiantes de primer ingreso en nuestra Licenciatura, con el objetivo de facilitar su inicio académico:
...
Solo necesito que me confirmés cuántas materias te gustaría llevar y procedo con el registro.', false, NULL);
  SELECT id INTO cat_id FROM public.categorias WHERE conjunto_id = new_conjunto_id AND name = 'Costos por carrera';
  INSERT INTO public.mensajes (categoria_id, sub_categoria, title, excerpt, is_pinned, internal_note) VALUES
  (cat_id, NULL, 'Costo Licenciatura — activo / regular', '📘 Si sos estudiante de reingreso, este es un excelente momento para retomar tu carrera y avanzar hacia tus metas profesionales. En la Universidad Isaac Newton queremos que aprovechés esta oportunidad y por eso contamos con beneficios especiales para vos:
...
¿Deseás que avancemos con tu reingreso? 🚀💙', false, NULL);
  SELECT id INTO cat_id FROM public.categorias WHERE conjunto_id = new_conjunto_id AND name = 'Costos por carrera';
  INSERT INTO public.mensajes (categoria_id, sub_categoria, title, excerpt, is_pinned, internal_note) VALUES
  (cat_id, NULL, 'Costo Maestría — todas las modalidades', 'Para este cuatrimestre, la Universidad Isaac Newton ha habilitado beneficios especiales para los estudiantes de nuestra Maestría, con el objetivo de facilitar su inicio académico:
...
Si deseás asegurar tu cupo, puedo ayudarte a completar tu proceso de matrícula de inmediato.', false, 'Aplica para nuevo ingreso, reingreso y estudiante activo/regular de la Maestría en Ingeniería Ambiental.');

  -- CATEGORÍA: Proceso de matrícula en Acamsys
  INSERT INTO public.categorias (conjunto_id, name, color, icon) VALUES (new_conjunto_id, 'Proceso de matrícula en Acamsys', 'bg-purple', 'mail');
  SELECT id INTO cat_id FROM public.categorias WHERE conjunto_id = new_conjunto_id AND name = 'Proceso de matrícula en Acamsys';
  INSERT INTO public.mensajes (categoria_id, sub_categoria, title, excerpt, is_pinned, internal_note) VALUES
  (cat_id, NULL, 'Cómo matriculan los estudiantes regulares', '🔹 1. Ingreso al portal
...
También cuento con la guía oficial de uso de Acamsys, con pasos ilustrados. Si la necesitás, te la comparto con gusto.', false, NULL);
  ...
  -- CATEGORÍA: Seguimiento y trámites administrativos
  INSERT INTO public.categorias (conjunto_id, name, color, icon) VALUES (new_conjunto_id, 'Seguimiento y trámites administrativos', 'bg-indigo', 'map-pin');
  SELECT id INTO cat_id FROM public.categorias WHERE conjunto_id = new_conjunto_id AND name = 'Seguimiento y trámites administrativos';
  INSERT INTO public.mensajes (categoria_id, sub_categoria, title, excerpt, is_pinned, internal_note) VALUES
  (cat_id, NULL, 'Seguimiento a prospecto contactado', '¡Hola de nuevo! 😄
...
Me avisás cuando lo llenás y enviás, por favor 😉', false, 'Este mensaje lo envía el personal de Ventas cuando el prospecto ya está listo para formalizar. Incluir el link del formulario de la sede correspondiente, según lo indicado por TI.');
  SELECT id INTO cat_id FROM public.categorias WHERE conjunto_id = new_conjunto_id AND name = 'Correos del proceso de matrícula';
  INSERT INTO public.mensajes (categoria_id, sub_categoria, title, excerpt, is_pinned, internal_note) VALUES
  (cat_id, NULL, 'Correo de pre-matrícula — lo redacta Registro y Plataforma', 'Hola [Nombre del estudiante],
...
Con entusiasmo,
[Nombre del asesor]', false, 'Se envía por correo una vez que el prospecto llenó el formulario. Pedirle que devuelva el correo firmado. El punto sobre laboratorios solo aplica si la carrera los incluye.Asunto sugerido: ¡Asegura tu lugar hoy y vive una experiencia extraordinaria en la UIN!');
  SELECT id INTO cat_id FROM public.categorias WHERE conjunto_id = new_conjunto_id AND name = 'Correos del proceso de matrícula';
  INSERT INTO public.mensajes (categoria_id, sub_categoria, title, excerpt, is_pinned, internal_note) VALUES
  (cat_id, NULL, 'Correo de bienvenida tras confirmar el pago', 'Hola [Nombre del estudiante].
...
https://teams.microsoft.com/meet/250763169646734?p=hmLfwqjWPZH6J1nQyb', false, 'Enviar una vez confirmado el pago y formalizada la matrícula. Recordar generar la contraseña del estudiante en Acamsys.Asunto sugerido: ¡Bienvenid@ a la UIN! Tu matrícula está lista para un cuatrimestre extraordinario');
END $$;